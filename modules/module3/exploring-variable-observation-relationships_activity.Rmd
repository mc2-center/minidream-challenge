...
data into R variables that we can work with.



```{r}
data_dir <- "/home/rstudio/shared/data"    # The data we are using is located in this directory
...
To get a peek into how the data are organized, we can look at the names
of the columns.  Since we saw that these data frames are rather big, I'm
...
to 140,000.  However, most of the values are close to 0, and there's
...
print(sprintf("Num values > 10,000: %g", sum(X > 10000)))
print(sprintf("%% values > 10,000: %.2f", sum(X>10000)/length(X)*100))
```
...
right tail.  Nearly all genes have *low* expression, and very few genes
have *high* expression.  Unfortunately, many statistical tests do not
work well with this kind of distribution.  Instead, it is very common
to assume a Gaussian or normal distribution [^normal_dist].
...
distribution of the expression of genes in each sample.  (To simplify
the plot, I've only plotted 1 out of every 10 samples, as indicated in
the `I` variable).  When we look at these plots, we hope to see a
...
distribution should be similar.  If we see differences (i.e. some
samples have very different expression values), we will need to
investigate why that is so, whether it is due to a difference in the
biology (e.g. different tissues can exhibit different expression), or
a technical artifact (rather common, unfortunately).  Here, the data
looks pretty clean.  You can also try plotting out the entire data set
to see.
...
And now submit the answer below
