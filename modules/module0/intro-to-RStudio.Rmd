...
date: "Publish date: 2022-06-23 <br/>Updated on: `r format(Sys.time(), '%Y-%m-%d')`"
output:
...
### Welcome to the miniDREAM - Intro to R session! {.unlisted .unnumbered}
...
Aside from the built in functions, there are other packages that the R community has created that expand the functionality of R. Lets use one of those packages, `ggplot2`, to visualize another dataset. We'll also assign the `ChickWeight` dataset to the variable `chk` so we don't have to type as much.
...
You're now ready to submit! Before you can do so, you must first log in with your Synapse credentials. For security purposes, we recommend using a Synapse Personal Access Token (PAT) rather than your username and password. This token can be generated on the Synapse platform [here](https://www.synapse.org/#!PersonalAccessTokens:).

Click on the blue **Create New Token** button. You will be asked to provide a token name - we recommend using something descriptive such as "minidream". Under **Token Permissions**, give the token **View** and **Modify** permissions.  Upon generating, you will be given a token that is ONLY AVAILABLE FOR VIEWING ONCE. Copy the token, then paste it into `authToken` in the code chunk below.

> Note: By using `rememberMe = TRUE` in the login below, you will not need to provide your credentials from this point forward.

Once you copy over your PAT, go ahead and run the code chunk below.

```{r eval=FALSE}
library(synapser)
synLogin(authToken = "replace-me",
         rememberMe = TRUE,
         silent = TRUE)
submission <- submit_module_answers(module = 0)
```

If successful, a file with your prediction will be uploaded to Synapse and submitted to the challenge. You'll be able to see the results of your prediction on the miniDREAM scoreboards, with the submission ID that gets printed below.
