...
ANS: 6.7 (5-10 will be fine)

What is the mean module force (mforce_mean)? 
ANS: 21pN (20-22 will be fine)
...
ANS:
higher kon, clutch will not unbound totally, higher mean force
higher koff, higher clutch unbound frequency, lower mean force
higher clutchNum, clutch will not unbound totally, higher mean force
higher clutchK, higher clutch unbound frequency, higher mean force
higher actinFlow, higher clutch unbound frequency

Is module force = clutch bound number * clutchK? If not, why?
ANS: no, module force = sum (clutch_dx*clutchK)

What is the biological behavior represetned by this exercise?
ANS: When there is a F-actin flow due to myosin contraction, F-actin will stretch the clutches, force them to unbind from the substrate, and hence the retraction of protrusions
...
```{r}
knitr::include_graphics("clutch_x.png")
```

Based on the force balance, we can derive the substrate position,
```{r}
knitr::include_graphics("clutch_xsub.png")
```

Then the net stretch of the clutch is
```{r}
knitr::include_graphics("clutch_stretch.png")
```
...
```{r}
knitr::include_graphics("load_fail.gif")
```

, and its measured deformation is similar to simulation.
```{r}
knitr::include_graphics("exp_load_fail_deformation.png")
```


**Challenge Questions**
What do you observe?
ANS: We should see bound clutch number accumulates and unbound with time, so do module force and substrate deformation.

What is the approximate frequency for clutches to unbound (unbound_freq_deform)? ANS: 0.085/s (0.07-0.1 will be fine)

What is the mean module force (mforce_mean_deform)? 
ANS:80.8 pN (75-85 will be fine)
...
ANS: when subK is small, the system will have an equilibrium bound clutch number, and module force will keep increasing within a long period before it fails.
when subK is large, clutches will bind and unbind very frequently, with high unbound and module force frequency

When subK becomes very large, will the results become similar to Exercise 2?
ANS: yes

What is the biological behavior represented by this exercise?
ANS: the substrate stiffness can change the clutch adhesion dynamics, and further more change the protrusion dynamics, and cell migration


### stochastic modeling of motor-clutch mechanism
```{r}
knitr::include_graphics("motor_clutch_model.png")
```
...
```{r}
knitr::include_graphics("motor_clutch_results.png")
```
...
```{r}
knitr::include_graphics("CMS.png")
```
...
```{r}
knitr::include_graphics("CMS_result.png")
```
...
#synLogin('my_synapse_username', 'my_synapse_password', rememberMe = TRUE, silent = TRUE)
